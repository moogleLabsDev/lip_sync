...
def face_detect(images, resize_factor=1.0):
    detector = FaceAlignment(LandmarksType._2D, flip_input=False, device=device)

    batch_size = 16
    resized_images = []

    # Resize images if resize_factor is less than 1 (e.g., 0.5 for 50% smaller)
    ...
    while True:
        ...
        try:
            ...
            if batch_size == 1:
                raise RuntimeError('Image too big to run face detection on GPU. Please use a smaller --resize_factor value.')
            batch_size //= 2
    ...
    for rect, image in zip(predictions, resized_images):
        if rect is None:
    ...
    results = [[image[y1:y2, x1:x2], (y1, y2, x1, x2)] for image, (x1, y1, x2, y2) in zip(resized_images, boxes)]
...
def main():
	...
	rotate = False
	face_batch_size = 16
	output_path = 'Wav2Lip/results/result_video.mp4'
	...
	if not audio_path.endswith('.wav'):
		...
		command = 'ffmpeg -y -i {} -strict -2 {}'.format(audio_path, 'Wav2Lip/temp/temp.wav')

		subprocess.call(command, shell=True)
		audio_path = 'Wav2Lip/temp/temp.wav'
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
		...
		if i == 0:
			check_path = 'Wav2Lip/checkpoints/wav2lip_gan.pth'
	...
	command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_path, 'Wav2Lip/temp/result.avi', output_path)
